...
void getUniqueSubsetsHelper(vector<int>& arr, int index, vector<int>& subset, vector<vector<int>>& output){
    ...
    for(int i=index; i<arr.size(); i++){
        ...
        getUniqueSubsetsHelper(arr, i+1, subset, output);
...
vector<vector<int>> getUniqueSubsets(vector<int>& arr) {
    ...
    vector<int> subset;
    getUniqueSubsetsHelper(arr, 0, subset, output);
    return output;
}
